...
void setup() 
{
  ...
  /************************MODIFIED****************************/
  pwm_pin6.start(pwmPeriodPin6, 0);
  SerialUSB.println("Starting from 1kHz");
  /************************MODIFIED****************************/
...
void loop()
{
  ...
  if (percent >= 1) {
      ...
      SerialUSB.println(currentFrequency);

      if (pwm_pin6.set_period_and_duty(pwmPeriodPin6, 0)) {
          // changed
      } else {
          SerialUSB.println("Error!!! period is too large");
      }
  }

  pwm_pin6.set_duty((int)(pwmPeriodPin6 * percent));
